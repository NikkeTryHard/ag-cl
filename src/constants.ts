 ...
 * @returns Full path to the Antigravity state database
 */
 ...
 * @returns User-Agent in format "antigravity/version os/arch"
 */
...
export const ANTIGRAVITY_ENDPOINT_FALLBACKS: readonly string[] = [ANTIGRAVITY_ENDPOINT_DAILY, ANTIGRAVITY_ENDPOINT_PROD];
...
// Retry delay for 5xx errors and other transient failures
export const RETRY_DELAY_MS = 1000; // 1 second pause between retries
 ...
 * @param modelName - The model name from the request
 * @returns True if the model supports thinking blocks
 */
export function isThinkingModel(modelName: string): boolean {
  ...
  if (lower.includes("gemini")) {
    ...
    const versionMatch = /gemini-(\d+)/.exec(lower);
    if (versionMatch?.[1] !== undefined && parseInt(versionMatch[1], 10) >= 3) return true;
  }
  return false;
}

// Google OAuth configuration (from opencode-antigravity-auth)
export const OAUTH_CONFIG: OAuthConfigType = {
