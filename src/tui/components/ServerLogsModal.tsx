...
export function ServerLogsModal({ onClose }: ServerLogsModalProps): React.ReactElement {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToLogs((newLogs) => {
      setLogs([...newLogs]);
      if (autoScroll) {
        setScrollOffset(Math.max(0, newLogs.length - MAX_VISIBLE_LINES));
      }
    });
    return unsubscribe;
  }, [autoScroll]);
  ...
  useInput((input, key) => {
    ...
    if (key.downArrow) {
      setScrollOffset((o) => {
        const newOffset = Math.min(Math.max(0, logs.length - MAX_VISIBLE_LINES), o + 1);
        if (newOffset >= logs.length - MAX_VISIBLE_LINES) {
          setAutoScroll(true);
        }
        return newOffset;
      });
    ...
    if (key.pageDown) {
      setScrollOffset((o) => {
        const newOffset = Math.min(Math.max(0, logs.length - MAX_VISIBLE_LINES), o + MAX_VISIBLE_LINES);
        if (newOffset >= logs.length - MAX_VISIBLE_LINES) {
          setAutoScroll(true);
        }
        return newOffset;
    ...
    if (input === "g" && key.shift) {
      // Shift+G = go to end
      setAutoScroll(true);
      setScrollOffset(Math.max(0, logs.length - MAX_VISIBLE_LINES));
      return;
    }
  ...
  const visibleLogs = logs.slice(scrollOffset, scrollOffset + MAX_VISIBLE_LINES);
  ...
  return (
    <Box flexDirection="column" borderStyle="round" padding={1}>
      <Box marginBottom={1} justifyContent="space-between">
        ...
        <Text dimColor>
          {logs.length} entries {autoScroll ? "(auto-scroll)" : ""}
        </Text>
      </Box>

      {/* Log lines */}
      <Box flexDirection="column" height={MAX_VISIBLE_LINES}>
        ...
        {/* Pad empty lines */}
        {Array.from({ length: MAX_VISIBLE_LINES - visibleLogs.length }).map((_, i) => (
          <Text key={`empty-${String(i)}`}> </Text>
        ))}
      </Box>

      {/* Scroll indicator */}
      {logs.length > MAX_VISIBLE_LINES && (
        <Box marginTop={1}>
          <Text dimColor>
            Lines {scrollOffset + 1}-{Math.min(scrollOffset + MAX_VISIBLE_LINES, logs.length)} of {logs.length}
          </Text>
        </Box>
      )}
